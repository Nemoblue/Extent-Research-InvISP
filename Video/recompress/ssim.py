...
root_path = '/disk1/zhouji/recompress/'
out_path = '/disk1/zhouji/recompress/'
crf = 23

with open(out_path + 'lossy_%d_ssim.txt' % crf, 'w') as f:
    f.write('')
...
for idx, folder in enumerate(all_folders):
    ...
    print(folder.split('/')[-1] +
          ': %f ' % video_psnr + 'final: %f\n' % final_psnr)
    with open(out_path + 'lossy_%d_ssim.txt' % crf, 'a') as f:
