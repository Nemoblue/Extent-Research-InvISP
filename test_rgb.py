...
parser.add_argument("--jpeg_quality", type=int, default=90, help="JPEG quality for DiffJPEG. ")
args = parser.parse_args()
print("Parsed arguments: {}".format(args))

JPEG_quality = args.jpeg_quality
ckpt_name = args.ckpt.split("/")[-1].split(".")[0]
DiffJPEG = DiffJPEG(differentiable=True, quality=JPEG_quality).cuda()
...
def main(args):
    ...
    for i_batch, sample_batched in enumerate(tqdm(dataloader)):
        ...
        for i_patch in range(len(input_list)):
            ...
            pred.save(out_path+"pred_%s_%05d.jpg"%(file_name, i_patch), quality=JPEG_quality, subsampling=1)
            tar.save(out_path+"tar_%s_%05d.jpg"%(file_name, i_patch), quality=JPEG_quality, subsampling=1)
            tar_pred.save(out_path+"gt_pred_%s_%05d.jpg"%(file_name, i_patch), quality=JPEG_quality, subsampling=1)
